...
function playerTeamStrength(club, player, tacticKey) {
  ...
  let overallBoost = (player.overall - base) * 0.16; // cuanto por encima/debajo del plantel estas
  if (player.position === 'POR') {
    // Los porteros de calidad tienen un impacto determinante en la solidez del equipo
    const ref = attrOf(player, 'defensa');
    const fis = attrOf(player, 'fisico');
    overallBoost += ((ref + fis) / 2 - 50) * 0.08;
  }
...
function simulateMatch(player, club, opponentClub, tacticKey = 'equilibrado', options = {}) {
  const { bonusGoals = 0, bonusSave = false, bonusText = null, bonusMinute = null } = options;
  ...
  let oppGoals = rollGoals(oppStrength, myStrength);

  // Si el arquero ganó su jugada clave en el minijuego, salva un gol cantado
  if (bonusSave && oppGoals > 0) {
  ...
  const defending = attrOf(player, 'defensa');
  ...
  // Jugadas decisivas de minijuego
  if (player.position === 'POR') {
    if (bonusSave) {
      playerSaves++;
      events.push({
        ...
        type: 'atajada',
        text: bonusText || `🧤 ¡ATAJADÓN monumental de **${player.name}** que salva el gol!`
      });
    } else if (bonusText) {
      events.push({ minute: bonusMinute ?? rand(60, 88), type: 'fallo', text: bonusText });
    }

    // Atajadas adicionales durante el partido para el arquero
    const extraSaves = Math.max(0, Math.min(4, Math.floor((defending + physical) / 45) - oppGoals));
    for (let s = 0; s < extraSaves; s++) {
      playerSaves++;
      events.push({ minute: rand(5, 88), type: 'atajada', text: `🧤 Atajada segura de **${player.name}** bajo los tres palos.` });
    }
  } else {
    // Goles ganados en el minijuego de campo
    if (bonusGoals > 0) {
      myGoals += bonusGoals;
  ...
  // Tarjetas
  const cardBase = player.position === 'DEF' ? 0.2 : player.position === 'MED' ? 0.16 : player.position === 'POR' ? 0.04 : 0.11;
  let yellow = false;
  let red = false;
  ...
  if (!red && Math.random() < (player.position === 'POR' ? 0.005 : 0.02)) {
    red = true;
    events.push({ minute: rand(10, 88), type: 'roja', text: `🟥 ¡Tarjeta roja directa para **${player.name}**! (1 partido de suspensión).` });
  }

  // Lesion: Mansión de alto rendimiento reduce el riesgo al 50%
  let injuredMatches = 0;
  let injuryChance = Math.max(0.012, 0.06 - physical / 2200) * (player.age >= 32 ? 1.3 : 1);
  if (player.mansionPurchased) injuryChance *= 0.5;
  ...
  if (player.position === 'POR') {
    rating += playerSaves * 0.45;
    if (oppGoals === 0) rating += 1.4;
    else rating -= Math.min(1.5, oppGoals * 0.35);
  ...
  const cleanSheet = (player.position === 'POR' || player.position === 'DEF') && oppGoals === 0;
  const motm = rating >= 7.8 || playerGoals >= 2 || (player.position === 'POR' && cleanSheet && playerSaves >= 3);
